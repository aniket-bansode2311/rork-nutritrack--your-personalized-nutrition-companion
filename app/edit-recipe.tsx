...
import { Plus, Minus, Save, Search, X, ChefHat } from 'lucide-react-native';
...
export default function EditRecipeScreen() {
  ...
  const { recipes, updateRecipe, searchFoodItems, calculateRecipeNutrition } = useNutrition();
  ...
  useEffect(() => {
    if (recipe) {
      ...
      const ingredientInputs: IngredientInput[] = recipe.ingredients.map(ing => ({
        id: ing.id,
        foodItem: ing.foodItem,
        quantity: ing.quantity.toString(),
        unit: ing.unit,
      }));
      setIngredients(ingredientInputs.length > 0 ? ingredientInputs : [
        { id: '1', foodItem: null, quantity: '', unit: 'g' }
      ]);
    }
  }, [recipe]);
  ...
  const handleSave = () => {
    ...
    // Calculate nutrition
    const tempRecipe = { ...recipe, ...updates };
    const nutrition = calculateRecipeNutrition(tempRecipe);
    updates.nutritionPerServing = nutrition;
